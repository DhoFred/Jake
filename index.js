...
try{
		...
		if (update.connection == "open" || update.receivedPendingNotifications == "true") {
	        ...
	        console.log(color(`\n${themeemoji} YT CHANNEL: donbelovibel`,'magenta'))
            console.log(color(`${themeemoji} GITHUB: mohalicious `,'magenta'))
            console.log(color(`${themeemoji} INSTAGRAM: donbel_ovibel `,'magenta'))
            console.log(color(`${themeemoji} WA NUMBER: wa.me//254796032440`,'magenta'))
...
XeonBotInc.ev.on('creds.update', await saveCreds)
    ...
    let XeonBlokMsg = await XeonBotInc.sendTextWithMentions(XeonFucks.from, `*${XeonBotInc.user.name}* can't receive ${XeonFucks.isVideo ? `video` : `voice` } call. Sorry @${XeonFucks.from.split('@')[0]} you will be blocked. If accidentally please contact the owner wa.me//+254796032440 to be unblocked !`)
